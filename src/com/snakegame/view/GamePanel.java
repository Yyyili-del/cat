...
public class GamePanel extends JPanel {
    ...
    private Color backgroundColor1 = new Color(20, 30, 50);
    private Color backgroundColor2 = new Color(10, 20, 40);
    ...
    private void drawGame(Graphics g) {
        ...
        drawEffects(g);
        drawUI(g);
        if (gameState.isPaused()) drawPaused(g);
    }
    ...
    private void drawFoods(Graphics g) {
        ...
        for (FoodItem item : food.getFoodList()) {
            Point pos = item.getPosition();
            FoodType type = item.getType();
            g2d.setColor(new Color(type.getColor().getRed(), type.getColor().getGreen(), type.getColor().getBlue(), 100));
    ...
    private void drawEffects(Graphics g) {
        ...
        if (effect.equals("GOLDEN")) {
            ...
            int centerX = pos.x + UNIT_SIZE / 2;
            int centerY = pos.y + UNIT_SIZE / 2;
            int radius = (int)(15 + Math.random() * 10);
            ...
            for (int i = 0; i < 20; i++) {
                ...
                g2d.setColor(new Color(255, 100 + (int)(Math.random() * 155), 0, 200));
                g2d.fillOval(px - size/2, py - size/2, size, size);
            }
            for (int i = 0; i < 12; i++) {
                double angle = Math.toRadians(Math.random() * 360);
                int distance = (int)(Math.random() * 40);
                int px = centerX + (int)(Math.cos(angle) * distance);
                int py = centerY + (int)(Math.sin(angle) * distance);
    ...
    private void drawDefaultGameOver(Graphics2D g2d) {
        ...
        for (int i = 0; i < 150; i++) {
            int x = (int)(Math.random() * WIDTH);
            int y = (int)(Math.random() * HEIGHT);
            g2d.fillOval(x, y, 1 + (int)(Math.random() * 3), 1 + (int)(Math.random() * 3));
        }
        g2d.setStroke(new BasicStroke(3));
        ...
        g2d.drawRoundRect(15, 15, WIDTH - 30, HEIGHT - 30, 15, 15);
        String gameOverText = "GAME OVER";
        g2d.setFont(new Font("Monospaced", Font.BOLD, 52));
        FontMetrics fm = g2d.getFontMetrics();
        int textX = (WIDTH - fm.stringWidth(gameOverText)) / 2;
        g2d.setColor(new Color(0, 0, 0, 150));
        ...
        GradientPaint textGradient = new GradientPaint(textX, HEIGHT / 2 - 65, new Color(255, 80, 120), textX + fm.stringWidth(gameOverText), HEIGHT / 2 - 45, new Color(255, 180, 80));
